...
Page({
      ...
      data:{
       ...
       success:res=>{
         if(res.data.check_status==true){
           wx.navigateTo({
             url: '/pages/repeat/repeat?qr_code_str='+qr_code_str,
           })
         }else{
  ...
  sbmitpdf(imgPaths){
    ...
    var that = this;
    wx.uploadFile({
      url:'https://tzzs.toommi.com/ygapi/Invoice/invoice_import?uid='+wx.getStorageSync('uid'), //仅为示例，非真实的接口地址
      filePath:imgPaths,
      name: 'file_pdf',//示例，使用顺序给文件命名
      success:function(e){
        ...
        if(res.code==200){
         ...
         var invoice_verify_info=res.data.invoice_info;
          ...
          that.check_duplicate(invoice_verify_info.invoice_code,invoice_verify_info.invoice_num).then(resolve=>{
            if(resolve){
                  that.setData({
                    pdf:imgPaths,
                  ...
                  pdf_year:invoice_verify_info.invoice_date,
                  invoice_id:invoice_verify_info.id,
            ...
            if(resolve){
              ...
              this.setData({
                ...
                'info.tax_rate_list':res.data.basic_info.tax_rate_list,
                  ...
                  'formData.gfmc':invoice_verify_info.purchaser_name,
                  'formData.gfsbh':invoice_verify_info.purchaser_register_num,
                  'formData.bxry':res.data.basic_info.user_info.name,
                  'formData.bxgs':res.data.basic_info.user_info.unit,
                  'formData.bxbm':res.data.basic_info.user_info.department,
                  date:invoice_verify_info.invoice_date.split('年')[0]+invoice_verify_info.invoice_date.split('年')[1].split('月')[0]+invoice_verify_info.invoice_date.split('年')[1].split('月')[1].split('日')[0]
              })
            }
          })
...
check_duplicate(invoice_code,invoice_num){ //ocr发票识别查重
    ...
    data:{
     ...
     success:res1=>{
       if(res1.data.check_status==true){
         wx.navigateTo({
           url: '/pages/repeat/repeat?type=3'+'&invoice_code='+invoice_code+'&invoice_num='+invoice_num,
         })
         resolve(false);
       }else{
        resolve(true);
  ...
  basic_info(){ //基础列表
    http.basic_info({
      ...
      success:res=>{
        if(res.code==200){
          var info=res.data.basic_info
          this.setData({
            infolist:info.purchaser_name_list,
            'info.seller_name_list':(info.seller_name_list||[]).map(a=>{
              return a.name;
            }),
            'info.expense_type_list':(info.expense_type_list||[]).map(a=>{
              return a.name;
            }),
            'info.reimbursement_personnel_list':(info.reimbursement_personnel_list||[]).map(a=>{
              return a.name;
            }),
            'info.purchaser_name_list':(info.purchaser_name_list||[]).map(a=>{
              return a.name;
            }),
            'info.invoice_type_list':info.invoice_type_list,
            'info.tax_rate_list':info.tax_rate_list,
            'formData.bxry':info.user_info.name,
            'formData.bxgs':info.user_info.unit,
            'formData.bxbm':info.user_info.department,
            'formData.hj':this.formatDecimal((Number(this.data.formData.je)+Number(this.data.formData.se)),2) 
          })
          if(!this.data.kabao){
            this.setData({
              'formData.lx':info.invoice_type_list[0]
            })
          }
          if(this.data.result){
